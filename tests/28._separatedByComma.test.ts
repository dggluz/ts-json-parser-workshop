...
describe.skip('_separatedByComma', () => {
  it('Parses the empty string', async () => {
    ...
    const _separatedByCommaFooParser = _separatedByComma(fromStr('foo'));
    // ...and the empty string input
    const input = '';

    // WHEN:
    // Parsing the input with the _separatedByCommaFooParser
    const { result, remaining } = await _separatedByCommaFooParser(input);
